...
type Role = 'parent' | 'teen' | 'child' | 'toddler' | 'extended';
interface Member {
  name: string;
  role: Role;
  color: string;
...
const ROLES: { key: Role; label: string; emoji: string }[] = [
  { key: 'parent',   label: 'Родител',     emoji: '👨‍👩‍👧' },
...
interface StepFamilyProps {
  ...
  parentColor: string;
  setParentColor: (v: string) => void;
  familyName: string;
  setFamilyName: (v: string) => void;
  timezone: string;
  setTimezone: (v: string) => void;
}
function StepFamily({ parentName, setParentName, parentColor, setParentColor, familyName, setFamilyName, timezone, setTimezone }: StepFamilyProps) {
  return (
    <div className="animate-slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        <div style={{ fontFamily: 'Nunito, sans-serif', fontSize: 20, fontWeight: 800, color: T.text, marginBottom: 4 }}>
          Разкажете ни за себе си
        </div>
        <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: T.text2 }}>
          Вие сте първият родител на семейния акаунт.
        </div>
      </div>

      {/* Parent profile */}
      <div style={{ background: T.surf2, borderRadius: 16, padding: 16, border: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{
            width: 52, height: 52, borderRadius: '50%', background: parentColor, flexShrink: 0,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 20, fontWeight: 800, color: '#fff', fontFamily: 'Nunito, sans-serif',
          }}>
            {parentName ? initials(parentName) : '?'}
          </div>
          <div style={{ flex: 1 }}>
            <Input label="Вашето ime" placeholder="Мама / Иван" value={parentName} onChange={setParentName} />
          </div>
        </div>
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 8, fontFamily: 'DM Sans, sans-serif' }}>Цвят на аватара</div>
          <div style={{ display: 'flex', gap: 8 }}>
            {T.avatars.slice(0, 6).map(c => (
              <div key={c} onClick={() => setParentColor(c)} style={{
                width: 30, height: 30, borderRadius: '50%', background: c, cursor: 'pointer',
                border: parentColor === c ? `3px solid ${T.text}` : '3px solid transparent',
                boxShadow: parentColor === c ? `0 0 0 2px ${T.surface}, 0 0 0 4px ${c}` : 'none',
      ...
      {/* Timezone picker */}
      <div>
        <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 6, fontFamily: 'DM Sans, sans-serif' }}>Часова зона</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {TIMEZONES.map(tz => (
            <div key={tz.value} onClick={() => setTimezone(tz.value)} style={{
              ...
              <div style={{
                ...
                {timezone === tz.value && <div style={{ width: 8, height: 8, borderRadius: '50%', background: T.mustDo }} />}
              </div>
              <span style={{
  ...
  return (
    <div className="animate-slide-in" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div>
        ...
        <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 13, color: T.text2 }}>
          Деца, втори родител или разширено семейство. Вие вече сте добавен/а автоматично.
        </div>
      </div>
...
export default function OnboardingPage() {
  ...
  const [parentColor, setParentColor] = useState<string>(T.avatars[0]);
  ...
  const canProceed =
    step === 1 ? parentName.trim().length > 0 : true;
        ...
        <div style={{
          ...
          {step < totalSteps - 1 ? (
            <div
              ...
              style={{
                width: '100%', background: canProceed ? T.mustDo : T.border, color: '#fff',
                borderRadius: 16, padding: '16px 0',
                textAlign: 'center', fontSize: 16, fontWeight: 700,
                fontFamily: 'DM Sans, sans-serif',
                cursor: canProceed ? 'pointer' : 'not-allowed',
                boxShadow: canProceed ? `0 6px 22px ${T.mustDo}50` : 'none',
            ...
            <div
              onClick={async () => {
                ...
                setSaving(true);
                try {
                  await createFamily(familyName || 'Нашето семейство', dailyBase, parentName || 'Родител', parentColor, members);
                  router.push('/home');
                } catch {
                  setSaving(false);
                }
